...
class MultipartParam(object):
    ...
    def encode_hdr(self, boundary):
        ...
        if self.filesize is not None:
            headers.append("Content-Length: %s" % str(self.filesize))
        else:
            headers.append("Content-Length: %s" % str(len(self.value)))
...
class TransferChunkedEncoder:
    def __init__(self, stream):
        self.oldBuffer = ""
        self.lastChunkSent = False
        self.stream = stream
        self.totalInputLength = 0
        self.totalOutputLength = 0
        
    def read(self, size):            
        if len(self.oldBuffer) < size:
            buf = self.stream.read(size)
            self.totalInputLength += len(buf)
            if len(buf) == 0:
                if not self.lastChunkSent:
                    self.oldBuffer += "0\r\n\r\n"
                    self.lastChunkSent = True
            else:
                self.oldBuffer = self.oldBuffer + hex(len(buf))[2:] + "\r\n" + buf + "\r\n"
            
        if size > len(self.oldBuffer):            
            ret = self.oldBuffer
            self.oldBuffer = ""
        else:
            ret = self.oldBuffer[:size]
