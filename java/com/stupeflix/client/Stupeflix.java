...
public class Stupeflix extends StupeflixBase {
    protected final static String TEXT_XML_CONTENT_TYPE = "text/xml";
    protected final static String APPLICATION_ZIP_CONTENT_TYPE = "application/zip";
    protected final static String APPLICATION_URLENCODED_CONTENT_TYPE = "application/x-www-form-urlencoded";
    ...
    public Stupeflix(String accessKey, String secretKey, String host, String service, boolean debug)
    {
        super(accessKey, secretKey, debug);
        this.host = (host == null)  ? "services.stupeflix.com" : host;
        this.service = (service == null) ? "stupeflix-1.0" : service;
        this.base_url  = this.host + "/" + this.service;
    }
    ...
    {
        return new StupeflixConnection(this.host, this.service, this.debug);
    }
    ...
    public void sendDefinition(String user, String resource, String filename, String body) throws Exception
    {
        // Check parameters
        ...
        if (Stupeflix.isZip(filename, body)) {
            contentType = this.APPLICATION_ZIP_CONTENT_TYPE;
        } else {
            contentType = this.TEXT_XML_CONTENT_TYPE;
        }
    ...
    // @return         : True in case of success, will throw an exception otherwise
    public void getDefinition(String user, String resource, String filename) throws Exception
    {
        // Check parameters
        ...
        // Build the request url
        String url = this.definitionUrl(user, resource);
        // Get the content
    ...
    public void createProfiles(String user, String resource, Object profiles) throws UnsupportedEncodingException, Exception
    {
        // Create the base url
        String url = this.createProfilesUrl(user, resource);
        String xml = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>" + profiles.toString();
        String body = this.XML_PARAMETER + "=" + URLEncoder.encode(xml, "UTF-8");

        this.sendContent("POST", url, null, body, this.APPLICATION_URLENCODED_CONTENT_TYPE);
    }
    ...
    public void getProfile(String user, String resource, String profile, String filename) throws Exception
    {
        // Check parameters
    ...
    {
        String url = this.profilePreviewUrl(user, resource, profile);
        // Sign the url
    ...
    public String getProfileStatus(String user, String resource, String profile) throws Exception
    {
        String url = this.profileStatusUrl(user, resource, profile);
        StupeflixConnection connection = this.getContent(url, null);
        String body = new String(connection.getResponseBodyBytes());
    ...
    {
        String url = this.profileThumbUrl(user, resource, profile, "thumb.jpg");
        // Sign the url
        url = this.signUrl(url, "GET", "", "", null, false);
        // Add the base url
        return "http://" + this.base_url + url;
    ...
    public void answer_error(StupeflixConnection connection, String message) throws Exception
    {
        String body = new String(connection.getResponseBodyBytes());
    ...
    public void sendContent(String method, String url, String filename, String body, String contentType) throws Exception
    {
        // Check parameters
        if(url == null)
        {
        ...
        if (body != null)
        {
            ...
            size = Stupeflix.stringLength(body);
        }
        else
        {
        ...
        String md5base64 = (String) md5hashes.get("base64");

        // Build the headers array
        HashMap headers = new HashMap();
        headers.put(this.HEADER_CONTENT_MD5, md5base64);
        headers.put(this.HEADER_CONTENT_LENGTH, Long.toString(size));
        headers.put(this.HEADER_CONTENT_TYPE, contentType);
        ...
        String refEtag = (String) md5hashes.get("hex");
        String etag = (String) connection.getResponseHeaders().get("etag");
    ...
    public StupeflixConnection getContent(String url, String filename) throws Exception
    {
        // Check parameters
        if(url == null)
        {
        ...
        {
            md5hashes = this.md5string(new String(connection.getResponseBodyBytes()));
        }
